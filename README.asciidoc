...
Implementation::
    Try to write a single program, which is able to solve both
...
1. The most important aspect is **separation of concerns**. Think about the different concerns involved in the task. For example: one concern is reading the files. Try to design separate software components for each concern. 
